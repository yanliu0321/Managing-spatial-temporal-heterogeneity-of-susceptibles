%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%在文件夹MCMC中运行此程序
%或者先下载工作区ShanghaiMCMC_histor.mat，然后运行此程序
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear all
clc
load ShanghaiMCMC_history.mat
Data_case=csvread('infected_data.csv');
...
%xlswrite('MCMC参数估计的后验分布',chain)
figure
mcmcplot(chain,[],results,'pairs');
figure
mcmcplot(chain,[],results,'denspanel',2);
%%
%ps:参数估计结果,pCI:参数置信区间
...
% Uconfbeta=0.4875+tinv(0.95,n-1)*0.0001726;Lconfbeta=0.4875-tinv(0.95,n-1)*0.0001726;
% Uconfk=0.3020+tinv(0.95,n-1)*0.00082744;Lconfk=0.3020-tinv(0.95,n-1)*0.00082744;
% UconfI0=1.0015+tinv(0.95,n-1)*0.0014981;LconfI0=1.0015-tinv(0.95,n-1)*0.0014981;
% UconfSe0=3.9996e+05+tinv(0.95,n-1)* 43.717 ;LconfSe0=3.9996e+05-tinv(0.95,n-1)* 43.717 ;
...
legend('95% CI','Mechanism model','UDE model','data');ylabel('Daily new infections');
...
title('(b)','Position',[-13 2.5]);legend({'Mechanism model','UDE model','R_t=1'})
grid on
grid minor
...
legend({'Mechanism model','UDE model'});ylabel('Recruitment rate of susceptible');xlabel('Date(days)')
...
legend({'95% CI','Mechanism model','UDE model'});ylabel('Sr(t)');xlabel('Date(days)')
...
legend({'95% CI','Mechanism model','UDE model'});ylabel('Se(t)');xlabel('Date(days)')
...
legend({'95% CI','Mechanism model','UDE model'});ylabel('I(t)');xlabel('Date(days)')
...
legend({'95% CI','Mechanism model','UDE model'},'Location','northwest');ylabel('R(t)');xlabel('Date(days)')
set(gca,'XTick',[ 6 37 67 98 128]);xlim([1 129]);
%set(gca,'XTickLabel',{'','2018.01.31','2020.02.31'});
set(gca,'XTickLabel',{'2022/3/1','4/1','5/1','6/1','7/1'});
...
for i=1:3
   ...
   %Model 1
     ...
     [t,xx]=ode45(@SIRmodel1,1:0.1:n,XX0,[],par1,par2);
...
grid on
grid minor
axes('Position',[0.15 0.3 0.07 0.07]);
...
MmdatacumIrate=[0 r0; k' datacumIrate(1:3)' datacumIrate(4:6)' datacumIrate(7:9)' ]'
%Model1
...
grid on
grid minor
set(gcf,'unit','centimeters','position',[10 10 30 11])%设置窗口的大小，（10,3）起点坐标，16宽，14高
